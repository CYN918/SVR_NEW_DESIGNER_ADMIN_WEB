	...
	<div class="w ctcontent">
		<div style="background: white;">
			<div class="margin40 cttitle" style="height: 60px;line-height: 60px;margin:0px 10px 15px 10px;">
				...
				<div class="textcenter" style="width: 69%;float:left;">
					<span>
						<span 
						style="height: 58px;"
						@click="tabsChange(index,item.linkTo)"
						v-if="(adminuseraccess.indexOf(item.accessid) > -1)"
						:class="index == commonTopData.tabnums ? 'tabs tabactive' : 'tabs'"
						v-for="(item,index) in tnna">
							<el-badge
							:value="doCount[item.vnum] == 0 ? '' : doCount[item.vnum]" 
				...
				<div class="fright" style="float:left;"  v-if="commonTopData.option">
					<div class="paddinglr40 relative" style="height: 35px;border-bottom: 2px solid #f0f2f5;line-height: 35px;background: none;">
						<div class="textcenter" style="float: left;">
							<span style="height: 35px;" v-for="(item,index) in commonTopData.option" v-if="index < 3 && (adminuseraccess.indexOf(allId) > -1)" :key="item.linkTo" :class="index == commonTopData.mintabnums ? 'tabs tabactive' : 'tabs'" @click="tabsChanges(item.linkTo)">
								<el-badge class="badge">{{ item.name }}</el-badge>
							</span>
							<span style="height: 35px;" v-for="(item,index) in commonTopData.option" v-if="index == 3 && (adminuseraccess.indexOf('52') > -1)" :key="item.linkTo" :class="index == commonTopData.mintabnums ? 'tabs tabactive' : 'tabs'" @click="tabsChanges(item.linkTo)">
								<el-badge class="badge">{{ item.name }}</el-badge>
							</span>
						</div>
					</div>
	...
	export default {
		...
		data() {
			return {
				...
				firstId:[],
				tnna:[
					{name:"项目报名",linkTo:"/review/xmBm",accessid:"61",vnum:'signup'},
					{name:"项目选标",linkTo:"/review/xmXb",accessid:"62",vnum:'select'},
					{name:"作品发布",linkTo:"/review/publishWork",accessid:"12",vnum:'work_launch'},
					{name:"作品入围",linkTo:"/review/finalistsWork",accessid:"13",vnum:'work_shortlist'},
					{name:"作品录用",linkTo:"/review/employWork",accessid:"14",vnum:'work_hire'},
					{name:"项目验收",linkTo:"/review/projectreview/projectrepending",accessid:"16",vnum:'project_accept'},
					{name:"供稿人申请",linkTo:"/review/applyPerson",accessid:"15",vnum:'contributor_apply'}
				],
			};
		},
		methods: {
