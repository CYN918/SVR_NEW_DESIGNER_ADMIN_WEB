		...
		<div class="wh" ref="elememt" id="table" ><!-- style="height:calc(100% - 44px);overflow-y: scroll" -->
			<el-table ref="multipleTable" :reserve-selection="true" :data="tableDatas" tooltip-effect  :header-cell-style="cellStyle" style="width: 100%" :max-height="tableHeight" v-loading="loading" @selection-change="handleSelectionChange" @row-click="rowclick">
				...
				<el-table-column v-for="(item,index) in tableConfig.list" :key="index" :label="item.lable" :width="item.width">
					<template slot-scope="scope"  >
						...
						<span v-else-if="item.type == 'price'">{{ "￥" + formatMoney(scope.row[item.prop])  }}</span>
						<span v-else-if="item.type == 'baifen'">{{scope.row[item.prop] + "%" }}</span>
			...
			<div class="w" style="text-align: right;background: #FFFFFF;">
				...
				<el-pagination v-if="tableConfig.total != ''" class="sel-pagin" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentpage"
				 :page-sizes="[50, 100, 200, 500,1000,2000]" :page-size="pagesize" layout="prev, pager, next, sizes,jumper" :total="Number(tableConfig.total)">
				</el-pagination>
			</div>
		</div>
