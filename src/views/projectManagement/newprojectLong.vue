		...
		<div class="detailtitle">
			...
			<div class="employment" style="text-align: center;">
				<span>
					...
					<span :class="{'fontactive':!Isnextshow}">项目信息设置</span>
				</span>
			...
			<div v-show="!Isnextshow">
					...
					<ul>
						...
						<li class="margint23 ofh">
							...
							<el-upload class="upload" action="454535" :http-request="httprequest" :show-file-list="false">
								<button class="defaultbtn" style="margin-left: 0;position: relative;right: 115px;">{{ form['banner'] ? "重新上传" : "上传图片" }}</button>
						...
						<li class="margint23 ofh">
							...
							<el-upload class="upload" action="454535" :http-request="httprequestbanner" :show-file-list="false">
								<button class="defaultbtn" style="margin-left: 0;position: relative;right: 115px;">{{ form['detail_banner'] ? "重新上传" : "上传图片" }}</button>
				...
				<div style="border-top: 1px solid #E6E6E6;padding-top: 40px;margin-top: 40px;">
					...
					<ul>
						...
						<li class="margint23 ofh">
							<span class="fleft detailKey" style="line-height: 40px;">下架时间</span>
							<el-date-picker
							v-model="form['dismount_time']"
							type="datetime"
							:disabled="objstatus > 2 || typebtn == 1"
							value-format="yyyy-MM-dd HH:mm:ss"
							:placeholder="typebtn == 1 ? '你已指定制作人':'选择日期时间'" >
							</el-date-picker>
						</li>
						<li class="margint23 ofh">
		...
		<el-dialog title="请选择模板预设" :visible.sync="dialogTableVisible2" custom-class="sel-dialog">
			<div>
				...
				<div style="overflow-y:auto;">
					<common-table :screenConfig="screenConfig" :tableConfig="tableConfig" :tableDatas="tableData" :tableAction="tableAction"
					 ref="Tableddtem" style="height: 450px;"></common-table>
				</div>
				<div class="w textcenter">
...
>>>>>>> master-fix
		...
		methods: {
			...
			reject1(){
				...
				if(this.currentpageName == "新建长期项目"){
					this.$router.push({ path: '/projectManagement/projectList/newprojectLong', query: {urlDate: ''}});
				} else{
					this.$router.push({ path: '/projectManagement/projectList/newprojectLong', query: {urlDate: ''}});
				}
			...
			edit(){
				...
				if(!this.form['dismount_time'] && this.form.rule_type == 1){
					this.$message({
						message:"请填写下架时间"
					})
					return;
				...
				if(this.form['dismount_time'] != this.deadline){
					this.form.status = "1"
				}
			...
			getactivityinfo(){
				this.api.projectdetail({
					...
					this.deadline = da.dismount_time;
					this.form = da;
					this.form.project_type=1;
			...
			getdemandlist(){
				this.api.demandlist({
					...
					if(this.$route.query.row){
						...
						localStorage.setItem("newprojectLong",this.$route.query.row);
						this.getactivityinfo();
					}else{
						if(this.currentpageName == "编辑活动"){
							if(localStorage.getItem("newprojectLong")){
								this.rows = JSON.parse(localStorage.getItem("newprojectLong"));
							}
						}
